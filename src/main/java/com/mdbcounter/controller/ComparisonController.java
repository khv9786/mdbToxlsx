...
public class ComparisonController implements startContainer {
    private final ConsoleView view;
    private final DbCompareService dbCompareService;
    private final MdbService mdbService;

    public ComparisonController(ConsoleView view, DbCompareService dbCompareService, MdbService mdbService) {
        this.view = view;
        this.dbCompareService = dbCompareService;
    ...
    public void execute() {
        ...
        List<MdbTableInfo> allMdbTableInfos = mdbService.loadMdbData(mdbFiles);
        view.printLoadingTime(loadStart,"mdb 로드 시간: ");


        view.printMessage("mdb와 db 비교 중 입니다 . . . ");
        loadStart = System.currentTimeMillis();
        ComparisonResult result = dbCompareService.compareDbWithMdb(allMdbTableInfos);
    ...
    private void displayAndExportResults(ComparisonResult result) {
        ...
        if (dbCompareService.hasExcelData(result)) {
